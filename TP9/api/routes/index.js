...
router.get('/atores/:idAtor', function(req, res) {
  Atores.getAtor(req.params.idAtor)
    .then(dados => res.jsonp(dados))
    .catch(e => res.status(500).send(`Erro na listagem do filme ${req.params.idAtor}: ${e}`))
});
...
router.get('/personagens/:idPersonagem', function(req, res) {
  Personagens.getPersonagem(req.params.idPersonagem)
    .then(dados => res.jsonp(dados))
    .catch(e => res.status(500).send(`Erro na listagem do filme ${req.params.idAtor}: ${e}`))
});
